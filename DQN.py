...
class Agent():
    ...
    def learn(self):
        ...
        not_done_states = torch.cat([next_states[a] for a in not_done]).view(-1, 84, 84, 4)
        not_done_states = torch.permute(not_done_states, (0, 3, 1, 2))

        evaluate = self.target_net(states).gather(1, acts)
    ...
    def choose_action(self, state):
        with torch.no_grad():
            ...
            if r < self.epsilon:
                ...
                Q = self.target_net.forward(state).squeeze(0).detach()
    ...
    def check_max_Q(self):
        ...
        return torch.max(self.target_net(state.squeeze(0).detach())).item()
...
def test(env):
    ...
    np.save("./Rewards/DQN_test_rewards.npy", np.array(rewards))
